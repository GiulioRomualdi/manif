...
public:
   ...
   * @return A reference to this.
   */
  _Derived& operator =(TangentBase&& m);
   ...
   */
  template <typename _DerivedOther>
  _Derived& operator =(TangentBase<_DerivedOther>&& m);

  template <typename _EigenDerived>
  _Derived& operator =(Eigen::MatrixBase<_EigenDerived>&& v);
...
template <typename _Derived>
_Derived&
TangentBase<_Derived>::operator =(TangentBase&& t)
{
  coeffs() = std::move(t.coeffs());
  return derived();
}
...
template <typename _DerivedOther>
_Derived&
TangentBase<_Derived>::operator =(TangentBase<_DerivedOther>&& t)
{
  coeffs() = std::move(t.coeffs());
  return derived();
}
...
_Derived& TangentBase<_Derived>::operator =(
    Eigen::MatrixBase<_EigenDerived>&& v)
{
  coeffs() = std::move(v);
  return derived();
}
